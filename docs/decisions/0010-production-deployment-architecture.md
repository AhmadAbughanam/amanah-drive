...
**Continuous deployment builds in GitHub Actions and only pulls on the VPS**, never builds there. `.github/workflows/ci-cd.yml` is one workflow file containing the whole pipeline as separate jobs: `api`, `ai-service`, and `web` build/test on every push and pull request; `publish` (gated to `main` pushes, and only if every test job succeeded via `needs:`) builds and pushes images to GHCR tagged `main` (moving) and `sha-<short>` (permanently pinned, for rollback); `deploy` (needs `publish`) then deploys over SSH. Keeping this as one file with job-level `needs:` dependencies, rather than two files linked by `workflow_run`, makes the "never deploy failing code" guarantee and the pipeline ordering visible in a single place. This reuses the exact GHCR-publishing mechanism `release.yml` already established for numbered releases, just on a different trigger and tag scheme — `release.yml` stays its own file because it serves a genuinely distinct, manually-triggered purpose (deliberate versioned releases, documented in `docs/RELEASING.md`), not continuous deployment.
