...
public static class AgentToolMetadataCatalog
{
    public static AgentToolMetadata For(string name) => name switch
    {
        ...
        "create_file" => Create(name, "Create a non-empty UTF-8 text file in the user's Drive. Existing files are never overwritten.", "{\"type\":\"object\",\"properties\":{\"name\":{\"type\":\"string\"},\"content\":{\"type\":\"string\"},\"folderId\":{\"type\":[\"string\",\"null\"],\"format\":\"uuid\"}},\"required\":[\"name\",\"content\"]}"),
        ...
        "move_folder" => Create(name, "Move a folder to another folder or the root. The destination cannot be the folder itself or one of its descendants.", "{\"type\":\"object\",\"properties\":{\"folderId\":{\"type\":\"string\",\"format\":\"uuid\"},\"destinationFolderId\":{\"type\":[\"string\",\"null\"],\"format\":\"uuid\"}},\"required\":[\"folderId\"]}"),
        "delete_file" => Create(name, "Permanently delete a file. Use only when the user clearly requested deletion.", "{\"type\":\"object\",\"properties\":{\"fileId\":{\"type\":\"string\",\"format\":\"uuid\"}},\"required\":[\"fileId\"]}"),
        "delete_folder" => Create(name, "Permanently delete a folder, all descendant folders, and every contained file. Use only when the user clearly requested deletion.", "{\"type\":\"object\",\"properties\":{\"folderId\":{\"type\":\"string\",\"format\":\"uuid\"}},\"required\":[\"folderId\"]}"),
